...
const translations = {
  en: {
    ...
    paid: 'PAID',
    feeReceipt: 'SALARY PAYMENT RECEIPT',
    studentDetails: 'Staff Details',
    studentName: 'Teacher Name',
    class: 'Designation',
    session: 'Salary Month',
    paymentBreakdown: 'Payment Breakdown',
    totalFee: 'Total Monthly Salary',
    installmentPaid: 'Installment Paid',
    totalAmountPaid: 'Total Amount Paid',
    outstandingBalance: 'Remaining Balance',
    grandTotalPaid: 'Amount Paid Now',
    remainingBalance: 'Remaining Balance',
    authorizedSignatory: 'Authorized Signatory',
    footer: (date: string) => `Generated digitally on ${date}. This record shows the salary payment details.`,
  ...
  hi: {
    ...
    paid: 'जमा',
    feeReceipt: 'वेतन भुगतान रसीद',
    studentDetails: 'स्टाफ का विवरण',
    studentName: 'शिक्षक का नाम',
    class: 'पद',
    session: 'वेतन का महीना',
    paymentBreakdown: 'भुगतान का विवरण',
    totalFee: 'कुल मासिक वेतन',
    installmentPaid: 'जमा किस्त',
    totalAmountPaid: 'कुल जमा राशि',
    outstandingBalance: 'शेष राशि',
    grandTotalPaid: 'अभी जमा की गई राशि',
    remainingBalance: 'बकाया राशि',
    authorizedSignatory: 'अधिकृत हस्ताक्षर',
    footer: (date: string) => `${date} को डिजिटल रूप से बनाया गया। यह रिकॉर्ड वेतन भुगतान के लिए किए गए सभी भुगतानों को दर्शाता है।`,
...
function ReceiptDocument({ teacherName, subject, monthName, year, amountPaidNow, totalSalary, totalPaidThisMonth, balance, note, schoolName, schoolAddress, schoolMobile, payments, lang = 'hi', receiptId }: Props) {
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View>
          ...
          <View style={styles.row}>
            <Text style={styles.label}>{t.session}</Text>
            ...
            <View key={p.id} style={styles.row}>
              <View>
                ...
                <Text style={{ fontSize: 9, color: '#71717A', marginTop: 1 }}>
                  {new Date(p.paid_at).toLocaleDateString(lang === 'hi' ? 'hi-IN' : 'en-IN', { day: 'numeric', month: 'short', year: 'numeric' })} {p.note ? `• ${p.note}` : ''}
                </Text>
              </View>
          ...
          <View style={[styles.row, { borderBottom: 0, marginTop: 12 }]}>
            ...
            <Text style={styles.valueGreen}>₹{totalPaidThisMonth.toLocaleString('en-IN')}</Text>
          </View>
          <View style={[styles.row, { borderBottom: 0 }]}>
            <Text style={[styles.label, { fontWeight: 'bold', color: '#18181B' }]}>{t.outstandingBalance}</Text>
            <Text style={balance > 0 ? styles.valueRed : styles.valueGreen}>
        ...
        <View style={styles.totalBox}>
          <View>
            ...
            <Text style={styles.totalBoxValue}>₹{amountPaidNow.toLocaleString('en-IN')}</Text>
            {note && <Text style={{ fontSize: 9, color: '#7C3AED', marginTop: 4 }}>Note: {note}</Text>}
          </View>
          <View>
            <Text style={styles.balanceLabel}>{t.remainingBalance}</Text>
            <Text style={styles.balanceValue}>₹{Math.max(balance, 0).toLocaleString('en-IN')}</Text>
          </View>
        </View>
...
function StaffReceiptHTML({ teacherName, subject, monthName, year, amountPaidNow, totalSalary, totalPaidThisMonth, balance, note, schoolName, schoolAddress, schoolMobile, payments, lang = 'hi', receiptId }: Props) {
  const t = translations[lang]
  const date = new Date().toLocaleDateString(lang === 'hi' ? 'hi-IN' : 'en-IN', { day: 'numeric', month: 'long', year: 'numeric' })
  const totalPayable = totalSalary
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f4f4f5' }}>
          ...
          <span style={{ fontSize: '13px', fontWeight: '700' }}>{teacherName}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f4f4f5' }}>
          <span style={{ fontSize: '13px', color: '#71717a' }}>{t.class}</span>
          <span style={{ fontSize: '13px', fontWeight: '700' }}>{subject}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f4f4f5' }}>
          <span style={{ fontSize: '13px', color: '#71717a' }}>{t.session}</span>
          <span style={{ fontSize: '13px', fontWeight: '700' }}>{monthName} {year}</span>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f4f4f5' }}>
          ...
          <span style={{ fontSize: '13px', fontWeight: '700' }}>₹{totalSalary.toLocaleString('en-IN')}</span>
        </div>
          ...
          payments.slice(0, 5).map((p) => (
            <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f4f4f5' }}>
              <div>
                ...
                <p style={{ fontSize: '11px', color: '#71717a', margin: '2px 0 0' }}>{new Date(p.paid_at).toLocaleDateString(lang === 'hi' ? 'hi-IN' : 'en-IN', { day: 'numeric', month: 'short', year: 'numeric' })} {p.note ? `• ${p.note}` : ''}</p>
              </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0 8px' }}>
          ...
          <span style={{ fontSize: '13px', fontWeight: '800', color: '#16a34a' }}>₹{totalPaidThisMonth.toLocaleString('en-IN')}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 0 12px' }}>
          ...
          <span style={{ fontSize: '13px', fontWeight: '800', color: balance > 0 ? '#dc2626' : '#16a34a' }}>
            ₹{Math.max(balance, 0).toLocaleString('en-IN')}
          </span>
        </div>
      </div>
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p style={{ fontSize: '14px', fontWeight: '800', color: '#7c3aed', margin: 0 }}>₹{amountPaidNow.toLocaleString('en-IN')}</p>
          {note && <p style={{ fontSize: '10px', color: '#7c3aed', marginTop: '4px' }}>Note: {note}</p>}
        </div>
        <div style={{ textAlign: 'right' }}>
           ...
           <p style={{ fontSize: '20px', fontWeight: '800', color: '#dc2626', margin: 0 }}>₹{Math.max(balance, 0).toLocaleString('en-IN')}</p>
        </div>
      </div>
...
export default function StaffReceiptPDF(props: Props) {
  ...
  async function handleWhatsApp() {
    ...
    let cleanedMobile = props.teacherMobile?.replace(/\D/g, '') || ''
    ...
    if (!props.teacherMobile) {
      setWhatsappError(lang === 'hi' ? 'शिक्षक का मोबाइल नंबर नहीं मिला' : 'Teacher mobile number not found')
    ...
    try {
      ...
      const baseFileName = `salary-receipt-${props.teacherName.replace(/\s+/g, '-').toLowerCase()}-${timestamp}`
      ...
      const message = lang === 'en'
        ? `Dear ${props.teacherName}, salary receipt for ${props.monthName} ${props.year}.\n\nAmount Paid: *₹${props.amountPaidNow.toLocaleString('en-IN')}*\nRemaining: *₹${Math.max(props.balance, 0).toLocaleString('en-IN')}*\n\n— ${props.schoolName}`
        : `प्रिय ${props.teacherName}, ${props.monthName} ${props.year} की वेतन रसीद।\n\nजमा राशि: *₹${props.amountPaidNow.toLocaleString('en-IN')}*\nशेष राशि: *₹${Math.max(props.balance, 0).toLocaleString('en-IN')}*\n\n— ${props.schoolName}`
        ...
        try {
          ...
          if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
              files: [file],
              title: `Salary Receipt: ${props.teacherName}`,
              text: fullMessage
            })
            setImgLoading(false)
            return
      ...
      const fallbackMessage = `Salary receipt for ${props.teacherName} - Amount: ₹${props.amountPaidNow}`
  ...
  function handleDownloadImage() {
    ...
    a.href = previewUrl
    a.download = `${props.teacherName}-salary-receipt-${lang}.jpg`
    a.click()
  }
    ...
    <div className="w-full space-y-4">
      ...
      <div style={{ position: 'absolute', left: '-9999px', top: '-9999px' }}>
        <div ref={receiptRef} style={{ display: 'none' }}>
          <StaffReceiptHTML {...props} lang={lang} receiptId={receiptId} />
        </div>
      </div>
      ...
      {previewUrl && mounted && createPortal(
        <div className="fixed inset-0 bg-black/70 z-[9999] flex items-center justify-center p-4 sm:p-6">
